 ...
 * @mixin \order
 */
class wlewallee_order extends wlewallee_order_parent {
			...
			if ($this->_validateTermsAndConditions()) {
				try {
					...
					if ($state === 'WALLEE_' . TransactionState::PENDING) {
						...
						throw new \Exception(
								WalleeModule::instance()->translate(
										"Order already exists. Please check if you have already received a confirmation, then try again."));
					}
					else {
						throw new \Exception(
								WalleeModule::instance()->translate("Unable to confirm order in state !state.", true,
										array(
											'!state' => $state 
										)));
					}
...
class_exists(\Wle\Wallee\Application\Model\Transaction::class);				$transaction = oxNew(\Wle\Wallee\Application\Model\Transaction::class);
				...
				if ($order->load($orderId) && $transaction->loadByOrder($orderId)) {
					...
					WalleeModule::getUtilsView()->addErrorToDisplay(
							WalleeModule::instance()->translate("An unknown error occurred, and the order could not be loaded."));
				}
			}
			else {
...
class_exists('oxorder');						$order = oxNew('oxorder');
	...
	public function getWalleeJavascriptUrl(){
		try {
			...
			WalleeModule::log(Logger::ERROR, $e->getMessage(), array(
				$this,
				$e 
			));
		}
